...
class Dean extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('Dean_model', 'dean_model');
		...
		if($this->session->userdata('dean_login') != 1){
	...
	public function promotions($param1 = '', $param2 = '', $param3 = ""){
		if ($param1 == 'student_list' && is_numeric($param2)) {
			$dean = $this->dean_model->get_dean_for_current_session();
			$data["students"] = $this->student_model->get_students_by_year_level_id_and_department_id($param2, $dean["department_id"]);
			$data["year_level"] = $this->crud_model->get_year_levels($param2);
			$this->load->view('backend/dean/promotions/student_list', $data );
		}

		if ($param1 == 'edit' && is_numeric($param2) ) {
			$data["record"] = $this->student_model->get_student_records($param2);
			$this->load->view('backend/dean/exam_student_records/edit', $data );
		}
		...
		if($param1 == "course_students_list" && is_numeric($param2) && is_numeric($param3)){
			...
			$data["course_id"] = $param2;
			$this->load->view('backend/dean/course_resource/list', $data);
		}
		
		if(empty($param1)){
			$page_data["year_levels"] = $this->crud_model->get_year_levels();
			$page_data['folder_name'] = 'promotions';
			$page_data['page_title'] = 'promotions';
			$this->load->view('backend/index', $page_data);
		}
	}
	// END of promotions
    ...
    public function vetting($param1 = '', $param2 = '', $param3 = ""){
		if ($param1 == 'list' && is_numeric($param2) ) {
			$dean = $this->dean_model->get_dean_for_current_session();
			$department_course = $this->dean_model->get_department_courses($dean["department_id"]);
			...
			if(is_numeric($param3)){
				//$response = $this->user_model->update_dean($dean["id"], ["department_id" => $param3]);
               
				$this->db->where('id',$dean["id"]);
				
				$this->db->update('deans', ["department_id" => $param3]);
			}
			$dean = $this->dean_model->get_dean_for_current_session();
			$data["academic_year"] = $this->crud_model->get_academic_years($academic_year_id);
			//$data["courses"] =  $this->lecturer_model->get_assigned_departments_by_user_id(user_id());
			$data["courses"] = $this->dean_model->get_department_courses($dean["department_id"]);
			$this->load->view('backend/dean/vetting/list', $data );
		}
		...
		if ($param1 == 'edit' && is_numeric($param2) ) {
			$data["record"] = $this->student_model->get_student_records($param2);
			$this->load->view('backend/dean/vetting_student_records/edit', $data );
		}
		...
		if ($param1 == 'approve_student_records' && $_SERVER["REQUEST_METHOD"] === "POST") {
            $data = $this->input->post(NULL, TRUE);
            $response = $this->lecturer_model->save_student_record($data);
            echo $response;
            return;
		...
		if ($param1 == 'bulk_approve_student_records' && $_SERVER["REQUEST_METHOD"] === "POST") {
            $data = $this->input->post(NULL, TRUE);
            $response = $this->lecturer_model->update_student_record($data, "bulk");
            echo $response;
            return;
		...
		if($param1 == 'course_students' && is_numeric($param2)){
			if(is_numeric($param3)){
				...
				// Redirect to current academic year
				return redirect( route( "vetting") ); 
			} 
			
			$page_data1['page_title'] = 'course_records';
			$page_data1['folder_name'] = 'vetting_student_records';
			...
			//$this->load->view('backend/dean/course_resource/list', $page_data1);
		}

		
		if(empty($param1)){
			$page_data['folder_name'] = 'vetting';
			$page_data['page_title'] = 'vetting';
			$this->load->view('backend/index', $page_data);
		}
	}
	//END EXAM section
    ...
    {
        if ($param1 == 'create') {
            $response = $this->crud_model->transcript_create();
            echo $response;
        }

        if ($param1 == 'update') {
            $response = $this->crud_model->transcript_update($param2);
            echo $response;
        }

        if ($param1 == 'delete') {
            $response = $this->crud_model->transcript_delete($param2);
            echo $response;
        }

        if ($param1 == 'list') {
            $page_data['student_id'] = $param2;
            return $this->load->view('backend/dean/transcript/list');
        }
		if ($param1 == 'pdf') {
			require_once 'application/libraries/dompdf/autoload.inc.php';
		  	        $semester_id = $this->crud_model->get_semesters();      
        		$academic_year = $this->crud_model->get_academic_years();  

						$page_data["transcripts"] = $this->student_model->get_students_transcript($param2,  $academic_year, $semester_id);

			$page_data["transcriptdata"] = $this->student_model->get_students_transcript($param2);
			...
			$dompdf->loadHtml($this->load->view('backend/dean/transcript/print', $page_data)->output->final_output);
