...
class Hod extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('Head_Of_Department_model', 'head_of_department_model');
		...
		if($this->session->userdata('head_of_department_login') != 1){
	...
	public function records($param1 = '', $param2 = '', $param3 = ""){
		if ($param1 == 'list' && is_numeric($param2) ) {
			...
			$this->load->view('backend/Head_of_Department/exam/list', $data );
		}
	...
	public function vetting($param1 = '', $param2 = '', $param3 = ""){
		if ($param1 == 'list' && is_numeric($param2) ) {
			...
			$hod = $this->head_of_department_model->get_head_of_department_for_current_session();
			$data["academic_year"] = $this->crud_model->get_academic_years($academic_year_id);
			//$data["courses"] =  $this->lecturer_model->get_assigned_departments_by_user_id(user_id());
			$data["courses"] =  $this->head_of_department_model->get_department_courses($hod["department_id"]);
			$this->load->view('backend/hod/vetting/list', $data );
		}

		if ($param1 == 'edit' && is_numeric($param2) ) {
			$data["record"] = $this->student_model->get_student_records($param2);
			$this->load->view('backend/hod/vetting_student_records/edit', $data );
		}
		...
		if($param1 == "course_students_list" && is_numeric($param2) && is_numeric($param3)){
			...
			$data["course_id"] = $param2;
			$this->load->view('backend/hod/course_resource/list', $data);
		}
		...
		if($param1 == 'course_students' && is_numeric($param2)){
			...
			$this->load->view('backend/hod/course_resource/list', $page_data1);
		}
