...
class Home extends CI_Controller {
	...
	// lecturerS PAGE
	function lecturers() {
		$count_lecturers = $this->db->get_where('users', array('role' => 'lecturer', 'school_id' => $this->active_school_id))->num_rows();
		$config = array();
		$config = manager($count_lecturers, 9);
		$config['base_url']  = site_url('home/lecturers/');
		$this->pagination->initialize($config);

		$page_data['per_page']    = $config['per_page'];
		$page_data['page_name']  = 'lecturer';
		$page_data['page_title'] = get_phrase('lecturers');
		$this->load->view('frontend/'.$this->theme.'/index', $page_data);
	}
