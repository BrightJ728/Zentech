...
class Login extends CI_Controller {
	...
	public function index(){
		if($this->session->userdata('superadmin_login') ==  true){
			...
			redirect(route('dashboard'), 'refresh');
		}elseif($this->session->userdata('lecturer_login') ==  true){
			redirect(route('dashboard'), 'refresh');
		}elseif($this->session->userdata('head_of_department_login') ==  true){
			redirect(route('dashboard'), 'refresh');
	...
	public function validate_login(){
		...
		if($user->num_rows() === 1 )
		{
			...
			if(password_verify($data["password"], $row->password)){
				...
				elseif($row->role == 'lecturer'){
					$this->session->set_userdata('lecturer_login', true);
					$this->session->set_userdata('user_id', $row->id);
					$this->session->set_userdata('school_id', $row->school_id);
					$this->session->set_userdata('user_name', $row->name);
					$this->session->set_userdata('user_type', 'lecturer');
					$this->session->set_flashdata('flash_message', get_phrase('welcome_back'));
					redirect(site_url('lecturer/dashboard'), 'refresh');
				}
				elseif($row->role == 'head of department'){
					$this->session->set_userdata('head_of_department_login', true);
					$this->session->set_userdata('user_id', $row->id);
					$this->session->set_userdata('school_id', $row->school_id);
					$this->session->set_userdata('user_name', $row->name);
					$this->session->set_userdata('user_type', 'hod');
					$this->session->set_flashdata('flash_message', get_phrase('welcome_back'));
					redirect(site_url('hod/dashboard'), 'refresh');
				}
				elseif($row->role == 'exam officer'){
					$this->session->set_userdata('dean_login', true);
					$this->session->set_userdata('user_id', $row->id);
					$this->session->set_userdata('school_id', $row->school_id);
					$this->session->set_userdata('school_id', $row->school_id);
					$this->session->set_userdata('user_name', $row->name);
					$this->session->set_userdata('user_type', 'dean');
					$this->session->set_flashdata('flash_message', get_phrase('welcome_back'));
					redirect(site_url('dean/dashboard'), 'refresh');
				}
		...
		//When details provided are wrong.
