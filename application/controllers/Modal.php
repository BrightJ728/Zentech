...
class Modal extends CI_Controller {
	...
	function popup($folder_name = '', $page_name = '' , $param1 = '' , $param2 = '', $param3 = '')
	{
		...
		if(isset($view) && $view !== null && !empty($view)){
			$data = $this->input->get(NULL, TRUE);
			$page_data['param1']		=	$data["param1"] ?? "";
			$page_data['param2']		=	$data["param2"] ?? "";
			$page_data['param3']		=	$data["param3"] ?? "";
			return $this->load->view( 'backend/'.$this->session->userdata('user_type').'/'.$folder_name.'/'. $view.'.php' ,$page_data );
		}
