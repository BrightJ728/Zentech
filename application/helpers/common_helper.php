...
if (!function_exists('grade_from_value')) {
	function grade_from_value($value)
	{
		if (is_numeric($value)) {
			$CI	= &get_instance();
			$grading_schemes = $CI->crud_model->get_grading_schemes();
			foreach ($grading_schemes as $scheme) {
				if ($value >= $scheme["lower_bound"] && $value <= $scheme["upper_bound"]) {
					return $scheme["grade"];
				}
			}
			return "N/A";
...
if (!function_exists('course_resource_path')) {
	function course_resource_path($src = '')
	{
		$path = "/uploads/course_resources/";
		return base_url($path . $src);
	}
}

if (!function_exists('course_resource_file_path')) {
	function course_resource_file_path($src = '')
	{
		$path = FCPATH . "/uploads/course_resources/";
		return $path . $src;
	}
}
...
if (!function_exists('get_menu')) {
	function get_menu($menu_id, $lookup_value)
	{
		$CI	= &get_instance();
...
// lecturer PERMISSION. PROVIDE MODULE NAME AND lecturerS ID
if (!function_exists('has_permission')) {
	function has_permission($class_id = "", $section_id = "", $module = "", $lecturer_id = "")
	{
		$CI = &get_instance();
		$CI->load->database();
		if (empty($lecturer_id)) {
			$user_id = $CI->session->userdata('user_id');
			$lecturer_details = $CI->db->get_where('lecturers', array('user_id' => $user_id))->row_array();
			$lecturer_id = $lecturer_details['id'];
		}
		$school_id = school_id();
		$permission_details = $CI->db->get_where('lecturer_permissions', array('class_id' => $class_id, 'section_id' => $section_id, 'lecturer_id' => $lecturer_id));
...
if (!function_exists('password')) {
	function password($str_password)
	{
		return password_hash($str_password, PASSWORD_BCRYPT);
	}
}
