...
class Frontend_model extends CI_Model {
  ...
  function get_frontend_lecturers() {
    $this->db->where('show_on_website', 1);
  ...
  function get_events() {
   ...
   public function get_event_image($image)
    {
        if (file_exists('uploads/images/event_images/'.$image)) {
            return base_url().'uploads/images/event_images/'.$image;
        } else {
            return base_url().'uploads/images/event_images/placeholder.png';
        }
    }
  // add event
  function event_create() {
   ...
   if ($_FILES['event_photo']['name'] != '') {
      ...
      move_uploaded_file($_FILES['event_photo']['tmp_name'], 'uploads/images/event_images/'. $data['image']);
  } else {
