        ...
        <div class="row">
          <div class="col-sm-6">
            <div class="float-left mt-1">
              ...
              <p class="text-muted font-13"><?= get_phrase('please_find_below_the_receipt'); ?>.</p>
            </div>
            ...
            <div class="mt-1 float-sm-right">
              ...
              <p class="font-13"><strong><?= get_phrase('status'); ?>: </strong>
                <?php if (strtolower($invoice_details['status']) == 'paid'): ?>
                  ...
                  <?php elseif (strtolower($invoice_details['status']) == 'unpaid'): ?>
                  <span class="badge badge-danger float-right"><?= get_phrase('unpaid'); ?></span></p>
                <?php else: ?>
                  <span class="badge badge-warning float-right"><?= get_phrase('partial_payment'); ?></span></p>
                <?php endif; ?>
            </div>
          </div><!-- end col -->
        </div>
                ...
                <thead>
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td><?= $invoice_details['currency_symbol'] ." ". number_format($invoice_details['total_amount']); ?></td>
                      <td>
					  	<h5><?= get_phrase('total_payment') ?>: <?= $invoice_details['currency_symbol'] . " ". number_format($invoice_details['paid_amount']); ?><h5>
                      ...
                      <td class="text-right"><?= $invoice_details['currency_symbol'] ;?>&nbsp;<?=  number_format($invoice_details['total_amount'] - $invoice_details['paid_amount']); ?></td>
                    </tr>
                  </tbody>
                </table>
          ...
          <div class="row">
            ...
            <div class="col-sm-6">
              <div class="float-right mt-1 mt-sm-0">
                <p><b><?= get_phrase('total_amount'); ?> :&nbsp;</b> <span class="float-right"><?= $invoice_details['currency_symbol'] ." ". number_format($invoice_details['total_amount']); ?></span></p>
                <p><b><?= get_phrase('due_amount'); ?> :&nbsp;<?= $invoice_details['currency_symbol'] ;?> <?= number_format($invoice_details['total_amount'] - $invoice_details['paid_amount']); ?></span></p>
               <h3> <?= $invoice_details['currency_symbol'] ;?>&nbsp;<?= number_format($invoice_details['total_amount'] - $invoice_details['paid_amount']); ?></h3>
              </div>
