...
$dean = $this->db->get_where('deans', array('user_id' => $user['id']))->row_array();
$social_links = json_decode($dean['social_links'], true);
?>
<form method="POST" class="d-block ajaxForm" action="<?= route('dean/update/'.$param1); ?>">
	<div class="form-row">
	<div class="form-group col-md-12">
		...
		<small id="" class="form-text text-muted"><?= get_phrase('provide_dean_name'); ?></small>
	</div>

	<div class="form-group col-md-12">
		<label for="email"><?= get_phrase('email'); ?></label>
		<input type="email" value="<?= $user['email']; ?>" class="form-control" id="email" name = "email" required>
		<small id="" class="form-text text-muted"><?= get_phrase('provide_dean_email'); ?></small>
	</div>
	...
	<div class="form-group col-md-12">
		...
		<select name="department_id" id="department_id" class="form-control select2" data-toggle = "select2" required>
			...
			foreach($departments as $department){
				?>
				<option value="<?= $department['id'] ?>" <?= $department['id'] == $dean['department_id']? 'selected' : "" ?>><?= $department['name']; ?></option>
			<?php } ?>
		</select>
	...
	<div class="form-group col-md-12">
		...
		<small id="" class="form-text text-muted"><?= get_phrase('provide_dean_phone_number'); ?></small>
	</div>
	...
	<div class="form-group col-md-12">
		...
		<small id="" class="form-text text-muted"><?= get_phrase('provide_dean_gender'); ?></small>
	</div>
	...
	<div class="form-group col-md-12">
		...
		<small id="" class="form-text text-muted"><?= get_phrase('provide_dean_address'); ?></small>
	</div>

	<div class="form-group col-md-12">
		<label for="about"><?= get_phrase('about'); ?></label>
		<textarea class="form-control" id="about" name = "about" rows="5" required><?= $dean['about']; ?></textarea>
	...
	<div class="form-group col-md-12">
		...
		<select name="show_on_website" id="show_on_website" class="form-control select2" data-toggle = "select2">
			<option value="1" <?php if($dean['show_on_website'] == 1) echo 'selected'; ?>><?= get_phrase('show'); ?></option>
			<option value="0" <?php if($dean['show_on_website'] == 0) echo 'selected'; ?>><?= get_phrase('do_not_need_to_show'); ?></option>
		</select>
		<small id="" class="form-text text-muted"><?= get_phrase('show_this_dean_on_website'); ?></small>
	</div>
	...
	<div class="form-group mt-2 col-md-12">
		<button class="btn btn-block btn-primary" type="submit"><?= get_phrase('update_dean'); ?></button>
	</div>
	</div>
</form>
<?php endforeach; ?>
