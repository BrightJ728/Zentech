...
<form method="POST" class="d-block ajaxForm" action="<?= route('frontend_gallery/update/'.$param1); ?>">
  <div class="form-row">
    ...
    <div class="form-group col-md-12">
        ...
        <select name="show_on_website" id="show_on_website" class="form-control select2" data-toggle = "select2">
            <option value="1" <?php if($gallery['show_on_website'] == 1) echo "selected"; ?>><?= get_phrase('show'); ?></option>
            <option value="0" <?php if($gallery['show_on_website'] == 0) echo "selected"; ?>><?= get_phrase('no_need_to_show'); ?></option>
        </select>
