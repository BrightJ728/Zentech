...
<div class="row ">
    <div class="col-xl-12">
        <div class="card">
            <div class="card-body">
                <h4 class="page-title">
                    <i class="dripicons-to-do title_icon"></i> <?= get_phrase('vetting'); ?>
                </h4>
	   ...
	   <div class="d-block form-group d-inline-block w-50">
       <label><?= get_phrase('department'); ?></label>
		<select name="department_id" id="department_id" class="form-control select2" data-toggle = "select2" required>
			...
			<?php $school_id = school_id();
            $departments= $this->dean_model->get_departments();
		...
		<small id="" class="form-text text-muted"><?= get_phrase('provide_a_department'); ?></small>
                ...
                <div class="form-group">
                    <button onclick="changeDepartment()" class="btn btn-primary"><?= get_phrase('change_department') ?></button>
                </div>
...
<script>
    ...
    function changeDepartment () {
        ...
        showDepartmentRecords(academic_year_id,  department_id);
    }
    async function showDepartmentRecords (academic_year_id, department_id) {
        let url = '<?= route("vetting/list/") ?>' + academic_year_id + '/' + department_id;
        let loader = document.querySelector("#gif-loader");
        loader.classList.remove("d-none");
        loader.classList.add("d-flex");
        $('.vetting_content').html("");
        $.ajax({
            type : 'GET',
            url: url,
            success : (response) => {
                $('.vetting_content').html(response);
                loader.classList.remove("d-flex");
                loader.classList.add("d-none");
            },
            error: (err) => {
                $('.vetting_content').html("<p>Sorry problem occurred while retrieving data</p>");
            } 
        });
    }

    async function showAcademicYearRecords (academic_year_id) {
        let url = '<?= route("vetting/list/") ?>' + academic_year_id;
        let loader = document.querySelector("#gif-loader");
        loader.classList.remove("d-none");
        loader.classList.add("d-flex");
        $('.vetting_content').html("");
