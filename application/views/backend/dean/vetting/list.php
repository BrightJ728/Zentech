    ...
    <div class="course-all">
        ...
        <div class="row clearfix" id="course-items">
            <?php foreach($courses as $course){ 
                $registered_students = $this->student_model->get_course_registered_students($course["id"], $academic_year['id']);
                ?>
                    ...
                    <div class="card mcard_4 border p-3 rounded shadow">
                        <div class="body">
                            ...
                            <div class="d-flex flex-wrap">
                                <span class="m-1">
                                    <a title="<?= get_phrase('exam') ?>" href="<?= route( "vetting/course_students/" . $course['id'] . "/"  . $academic_year['id']) ?>" class="btn btn-primary"><i class="fa fa-check-circle-o"></i> </a>
                                </span>
                                <span class="m-1">
                                    <a 
                                        ...
                                        onclick="return showAjaxModal('<?= route('courses/info/' . $course['id'])?>', '<?= get_phrase('course_details'); ?>');">
