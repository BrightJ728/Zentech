...
<div class="row">
    <div class="col-12">
        <div class="card pt-0">
            ...
            <form method="POST" class="col-12 d-block ajaxForm" action="<?= route('student/updated/'.$student_id.'/'.$student['user_id']); ?>" id = "student_update_form" enctype="multipart/form-data">
                <div class="col-md-12">
                    <div class="form-group row mb-3">
                        <label class="col-md-3 col-form-label" for="name"><?= get_phrase('name'); ?></label>
                        <div class="col-md-9">
                            <input type="text" id="name" name="name" class="form-control"  value="<?= $this->user_model->get_user_details($student['user_id'], 'name'); ?>" placeholder="name" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group row mb-3">
                        ...
                        <div class="col-md-9">
                            <input type="email" class="form-control" id="email" name="email" value="<?= $this->user_model->get_user_details($student['user_id'], 'email'); ?>" placeholder="email" required>
                        </div>
                    </div>
                        ...
                        <div class="col-md-9">
                            <select id="parent_id" name="parent_id" class="form-control select2"  data-toggle="select2" required >
                                ...
                                <?php foreach($parents as $parent): ?>
                                    <option value="<?= $parent['id']; ?>" <?php if($student['parent_id'] == $parent['id']) echo 'selected'; ?>><?= $this->user_model->get_user_details($parent['user_id'], 'name'); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-md-9">
                            <select name="class_id" id="class_id" class="form-control" required onchange="classWiseSectionOnStudentEdit(this.value)">
                                ...
                                <?php foreach($classes as $class){ ?>
                                    <option value="<?= $class['id']; ?>" <?php if($enroll['class_id'] == $class['id']) echo 'selected'; ?>><?= $class['name']; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-md-9" id = "section_content">
                            <select name="section_id" id="section_id" class="form-control" required >
                                ...
                                <?php foreach($sections as $section){ ?>
                                    <option value="<?= $section['id']; ?>" <?php if($enroll['section_id'] == $section['id']) echo 'selected'; ?>><?= $section['name']; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group row mb-3">
                        ...
                        <div class="col-md-9">
                            <input type="text" class="form-control date" id="birthdatepicker" data-toggle="date-picker" data-single-date-picker="true" name = "birthday"  value="<?= date('m/d/Y', $this->user_model->get_user_details($student['user_id'], 'birthday')); ?>" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group row mb-3">
                        ...
                        <div class="col-md-9">
                            <select name="gender" id="gender" class="form-control" required>
                                ...
                                <option value="Male" <?php if($this->user_model->get_user_details($student['user_id'], 'gender') == 'Male') echo 'selected'; ?>><?= get_phrase('male'); ?></option>
                                <option value="Female" <?php if($this->user_model->get_user_details($student['user_id'], 'gender') == 'Female') echo 'selected'; ?>><?= get_phrase('female'); ?></option>
                                <option value="Others" <?php if($this->user_model->get_user_details($student['user_id'], 'gender') == 'Others') echo 'selected'; ?>><?= get_phrase('others'); ?></option>
                            </select>
                        </div>
                    </div>

                    <div class="form-group row mb-3">
                        <label class="col-md-3 col-form-label" for="blood_group"><?= get_phrase('blood_group'); ?></label>
                        <div class="col-md-9">
                    ...
                    <div class="form-group row mb-3">
                        ...
                        <div class="col-md-9">
                            <textarea class="form-control" id="example-textarea" rows="5" name = "address" placeholder="address"><?= $this->user_model->get_user_details($student['user_id'], 'address'); ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-group row mb-3">
                        ...
                        <div class="col-md-9">
                            <input type="text" id="phone" name="phone" class="form-control" value="<?= $this->user_model->get_user_details($student['user_id'], 'phone'); ?>" placeholder="phone" required>
                        </div>
                    </div>
                    ...
                    <div class="text-center">
                        <button type="submit" class="btn btn-secondary col-md-4 col-sm-12 mb-4"><?= get_phrase('update_student_information'); ?></button>
                    </div>
                </div>
            </form>
        </div>
