...
<div class="row ">
    <div class="col-xl-12">
        <div class="card">
            <div class="card-body">
                <h4 class="page-title">
                    <i class="mdi mdi-chart-timeline title_icon"></i> <?= get_phrase('syllabus'); ?>
                    <button type="button" class="btn btn-outline-primary btn-rounded alignToTitle" onclick="rightModal('<?= site_url('modal/popup/syllabus/create'); ?>', '<?= get_phrase('create_syllabus'); ?>')"> <i class="mdi mdi-plus"></i> <?= get_phrase('add_syllabus'); ?></button>
                </h4>
            </div> <!-- end card body-->
        </div> <!-- end card -->
    </div><!-- end col-->
...
var showAllSyllabuses = function () {
    ...
    if(class_id != "" && section_id!= ""){
        $.ajax({
            url: '<?= route('syllabus/list/') ?>'+class_id+'/'+section_id,
            success: function(response){
