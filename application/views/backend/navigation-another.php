...
<div class="left-side-menu left-side-menu-detached content-main">
  ...
  <div class="leftbar-user">
    <a href="javascript: void(0);">
      <img src="<?= $this->user_model->get_user_image($this->session->userdata('user_id')); ?>" alt="user-image" height="42" class="rounded-circle shadow-sm">
      <?php
      $user_details = $this->user_model->get_user_details($this->session->userdata('user_id'));
      ?>
      <span class="leftbar-user-name"><?= $user_details['name']; ?></span>
    </a>
  </div>
  <!--- Sidemenu -->
  <ul class="metismenu side-nav side-nav-light">
    ...
    <li class="side-nav-item">
      <a href="<?= site_url($controller.'/dashboard'); ?>" class="side-nav-link">
    ...
    foreach($main_menus as $main_menu){
      ...
      if($check_menus->num_rows() > 0){ ?>
        <a href="javascript: void(0);" class="side-nav-link <?php if($main_menu['displayed_name'] == $navigate) echo 'active'; ?>">
          ...
          <span><?= get_phrase($main_menu['displayed_name']); ?></span>
        ...
        <ul class="side-nav-second-level" aria-expanded="false">
          ...
          foreach ($menus as $menu) {
            ...
            if($check_sub_menus->num_rows() > 0){ ?>
              <li class="side-nav-item">
                <a href="javascript: void(0);" aria-expanded="false"><?= get_phrase($menu['displayed_name']); ?>
                  <span class="menu-arrow"></span>
                </a>
                  ...
                  foreach ($sub_menus as $sub_menu) {
                    ...
                    <li>
                      <?php
                       ...
                       ?>
                      <a href="<?= site_url($route); ?>"><?= get_phrase($sub_menu['displayed_name']); ?></a>
                    </li>
                  <?php } ?>
                </ul>
              </li>
            <?php }else{ ?>
              <li class="<?php if($menu['displayed_name'] == $navigate) echo 'active'; ?>">
                <?php
                 ...
                 ?>
                <a href="<?= site_url($route); ?>" class="<?php if($menu['displayed_name'] == $navigate) echo 'active'; ?>"><?= get_phrase($menu['displayed_name']); ?></a>
              </li>
            <?php } ?>
          <?php } ?></ul><?php
        }else{ ?>
          <?php
           ...
           ?>
          <a href="<?= site_url($route); ?>" class="side-nav-link">
            <i class="<?= $main_menu['icon']; ?>"></i>
            <span><?= get_phrase($main_menu['displayed_name']); ?></span>
          </a>
        </li>
