...
<form method="POST" class="d-block ajaxForm" action="<?= route('manage_class/section/'.$param1); ?>">
    <?php $count = 0; ?>
    ...
    <?php foreach($sections as $section){
        ...
        <?php if($count == 1){ ?>
            <div class="form-row mr-2">
                <div class="form-group col-10">
                    <input type="hidden" class="form-control" id="section" name = "section_id[]" value="<?= $section['id']; ?>">
        ...
        <?php if($count != 1){ ?>
            <div class="form-row mr-2" id="sectionDatabase<?= $section['id']; ?>">
                <div class="form-group col-10">
                    <input type="hidden" class="form-control" id="section<?= $section['id']; ?>" name = "section_id[]" value="<?= $section['id']; ?>">
                    <input type="text" class="form-control" name = "name[]" value="<?= $section['name']; ?>" required>
                </div>
                <div class="form-group col-2">
                    <button class="btn btn-icon btn-danger" type="button" onclick="removeSectionDatabase('<?= $section['id']; ?>')"><i class="mdi mdi-window-close"></i></button>
                </div>
            </div>
        <?php } ?>
    ...
    <div class="form-group  col-md-12 p-0 mt-2">
        <button class="btn btn-block btn-primary" type="submit"><?= get_phrase('update'); ?></button>
    </div>
</div>
</form>
