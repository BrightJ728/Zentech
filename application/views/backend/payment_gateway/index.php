	...
	<div class="container">
		<div class="row justify-content-center mb-5">
			<div class="col-md-8">
				<div class="row">
					<div class="col-md-12">
						<span class="payment-header-text float-left"><b><?= get_phrase('make_payment'); ?></b></span>
						<a href="<?= route('invoice'); ?>" class="close-btn-light float-right"><i class="fa fa-times"></i></a>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="row justify-content-center">
			<div class="col-md-8">
				<div class="row">
					<div class="col-md-3">
						<p class="pb-2 payment-header"><?= get_phrase('payment'); ?> <?= get_phrase('gateway'); ?></p>

						<?php if ($paypal_activity[0]->paypal_active == 'yes'): ?>
							<div class="row payment-gateway paypal" onclick="selectedPaymentGateway('paypal')">
								<div class="col-12">
									<img class="tick-icon paypal-icon" src="<?= base_url('assets/payment/tick.png'); ?>">
									<img class="payment-gateway-icon" src="<?= base_url('assets/payment/paypal.png'); ?>">
								</div>
							</div>
						<?php endif; if ($stripe_activity[0]->stripe_active == 'yes'): ?>
							<div class="row payment-gateway stripe" onclick="selectedPaymentGateway('stripe')">
								<div class="col-12">
									<img class="tick-icon stripe-icon" src="<?= base_url('assets/payment/tick.png'); ?>">
									<img class="payment-gateway-icon" src="<?= base_url('assets/payment/stripe.png'); ?>">
								</div>
							</div>
						<?php endif; ?>
					...
					<div class="col-md-8">
						<div class="w-100">
							<p class="pb-2 payment-header"><?= get_phrase('invoice_list'); ?></p>
	                        <p class="item float-left">
								<span class="count-item"><?= get_phrase('1'); ?></span>
								<span class="item-title"><?= $invoice_details['title']; ?>
									<span class="item-price">
										<?php
											$total_amount_in_this_invoice = $invoice_details['total_amount'] - $invoice_details['paid_amount'];
										?>
										<?= currency($total_amount_in_this_invoice); ?>
									</span>
								</span>
								<span class="by-owner">
									<!-- owner name -->
						...
						<div class="w-100 float-left mt-4 indicated-price">
							<div class="float-right total-price"><?= currency($total_amount_in_this_invoice); ?></div>
						...
						<div class="w-100 float-left">
							<form action="<?= route('paypal_checkout'); ?>" method="post" class="paypal-form form">
								<hr class="border mb-4">
								<input type="hidden" name="invoice_id" value="<?= $invoice_details['id']; ?>">
								<button type="submit" class="payment-button float-right"><?= get_phrase('pay_by_paypal'); ?></button>
				            </form>
				            <form action="<?= route('stripe_checkout'); ?>" method="post" class="stripe-form form">
				            	<hr class="border mb-4">
								<input type="hidden" name="invoice_id" value="<?= $invoice_details['id']; ?>">
								<button type="submit" class="payment-button float-right"><?= get_phrase('pay_by_stripe'); ?></button>
				            </form>
				            <?php if(addon_status('payumoney') == 1): ?>
