            ...
            <?php foreach($marks as $mark):?>
                <?php if ($mark['student_id'] == $student_data['id']): ?>
                    <tr>
                        ...
                        <td><input class="form-control readonly" type="number" id="mark-<?= $mark['student_id']; ?>" name="mark" placeholder="mark" min="0" value="<?= $mark['mark_obtained']; ?>" required></td>
                        <td><input class="form-control readonly" type="text" id="comment-<?= $mark['student_id']; ?>" name="comment" placeholder="comment" value="<?= $mark['comment']; ?>"></td>
                    </tr>
                <?php endif; ?>
            <?php endforeach; ?>
        </tbody>
...
<script>
    function mark_update(student_id){
        ...
        let exam_id = '<?= $exam_id; ?>';
        let mark = $('#mark-' + student_id).val();
        let comment = $('#comment-' + student_id).val();
        if(subject_id != ""){
            $.ajax({
                type : 'POST',
