<?php 
    ...
    $fee = $this->accounting_m->get_resit_fees($param1);
?>
<form method="POST" class="d-block feesForm" action="<?= route('resit_fees/update/' . $param1); ?>">
    <div class="form-row">
        ...
        <div class="form-group col-md-12">
            ...
            <select required type="text" name="academic_year_id" class="form-control" id="academic_year_id" required>
                ...
                <?php foreach($academic_years as $academic_year){ ?>
                    <option <?= $fee["academic_year_id"] === $academic_year['id']? "selected": "" ?> value="<?= $academic_year['id']; ?>"><?= $academic_year['description']; ?></option>
                <?php } ?>
            </select> 
            <small id="academic_year_id_help" class="form-text text-muted"><?= get_phrase('provide_academic_year'); ?></small>
        </div>
        <div class="form-group col-md-12">
            <label for="program_id"><?= get_phrase('program'); ?></label>
            ...
            <small id="department_id_help" class="form-text text-muted"><?= get_phrase('select_programme'); ?></small>
        </div>
        ...
        <div class="form-group col-md-12">
            ...
            <select required type="text" name="year_level_id" class="form-control" id="year_level_id" required>
                ...
                <?php foreach($year_levels as $year_level){ ?>
                    <option <?= $fee["year_level_id"] === $year_level['id']? "selected": "" ?> value="<?= $year_level['id']; ?>"><?= $year_level['name']; ?></option>
                <?php } ?>
            </select> 
            <small id="department_id_help" class="form-text text-muted"><?= get_phrase('provide_department'); ?></small>
        </div>
        <div class="form-group col-md-12">
            ...
            <select required type="text" name="student_type" class="form-control" id="student_type" required>
                <option value=''><?= get_phrase("choose") ?>...</option>
                <option <?= $fee["student_type"] === 'local' ? "selected": "" ?> value='local'><?= get_phrase("local_student") ?></option>
                <option <?= $fee["student_type"] === 'foreign' ? "selected": "" ?> value='foreign'><?= get_phrase("foreign_student") ?></option>
                
            </select> 
            <small id="student_type_help" class="form-text text-muted"><?= get_phrase('provide_student_type'); ?></small>
        </div>
        <div class="form-group col-md-12">
            <label for="fee"><?= ucwords(get_phrase('resit_fee')); ?></label>
                ...
                <div class="input-group-prepend" >
                    <select required type="text" name="currency_id" class="form-control" id="currency_id" style="border-radius: 0.25em 0px 0px 0.25em;" required>
                        <?php foreach($currencies as $currency){ ?>
                            <option <?= $fee["currency_id"] === $currency['id']? "selected": "" ?> value="<?= $currency['id']; ?>"><?= $currency['symbol']; ?></option>
                        <?php } ?>
                    </select> 
                </div>
                <input type="number" step="0.01" class="form-control" aria-label="Text input with dropdown button" id="fee" name="fee" value="<?= $fee["amount"] ?>" required>
            </div>
            <small id="currency_id_help" class="form-text text-muted"><?= get_phrase('enter_resit_fees'); ?></small>
        </div>
        

        <div class="form-group  col-md-12">
            <button class="btn btn-block btn-primary" type="submit"><?= get_phrase('update_fee'); ?></button>
        </div>
    </div>
</form>
