...
<form method="POST" class="d-block ajaxForm" action="<?= route('school_fees/update/'.$param1); ?>">
    <div class="form-row">
        ...
        <select name="program_id" id="program_id" class="form-control select2" data-toggle="select2"  required onchange="getProgramStudents(this.value)">
            <option value=""><?= get_phrase('select_a_program'); ?></option>
            <?php foreach($programs as $program): ?>
            <option <?= $invoice_details['program_id'] == $program["id"] ? "selected": "" ?> value="<?= $program['id']; ?>"><?= $department['name']; ?></option>
            <?php endforeach; ?>
        </select>
        </div>
        <div class="form-group col-md-12">
